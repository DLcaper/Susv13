...
module.exports = {
  name: "embed",
  description: "tạo embed sử dụng lệnh",
  run: async (client, message, args) => {
    if (!message.member.premissions.has("MANAGER_GUILD")) return;
    let channel = message.mentions.channels.first();
    if (!channel) return message.reply("Vui lòng cung cấp kênh ** hợp lệ ** để gửi nhúng")
    ...
    let opt = new component().addComponents(
      new menu()
        ...
        .addOptions([
          {
            label: "Title",
            description: "Đặt tiêu đề cho việc nhúng",
            value: "etitle"
          },
          {
            label: "Description",
            description: "Đặt mô tả cho việc nhúng",
            value: "edesc"
          },
          {
            label: "Color",
            description: "Đặt màu cho nhúng",
            value: "ecolor"
          },
          {
            label: "Footer",
            description: "Đặt văn bản chân trang cho nhúng",
            value: "efooter"
          },
          {
            label: "Thumbnail",
            description: "Đặt một hình thu nhỏ cho việc nhúng",
            value: "ethumb"
          },
          {
            label: "Image",
            description: "Đặt hình ảnh cho nhúng (hình ảnh lớn ở giữa dưới)",
            value: "eimg"
          },
          {
            label: "Author",
            description: "Đặt văn bản tác giả cho nhúng (tiêu đề văn bản trên)",
            value: "eauthor"
          },
          {
            label: "URL",
            description: "Đặt URL cho nhúng (siêu liên kết cho tiêu đề)",
            value: "eurl"
          },
          {
            label: "Timestamp",
            description: "Đặt dấu thời gian cho việc nhúng (bên cạnh chân trang)",
            value: "etime"
          }
        ])
    )
    ...
    let cr = new embed()
      ...
      .setDescription("Vui lòng chọn một tùy chọn bên dưới để đặt giá trị cho nhúng")
      .setColor("GOLD")
    ...
    colb.on("collect", async (i) => {
      if (i.customId === "embc") {
        if (conf === false) return i.reply({
          content: "Bạn cần đặt mô tả vào nhúng trước khi bạn có thể gửi nó!",
          ephemeral: true
        })
        i.channel.send(`Nice! embed created in ${channel}!`)
        msg.delete();
        preview.delete();
        channel.send({
          embeds: [result]
